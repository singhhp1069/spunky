...
func networkWithRewardsObjects(t *testing.T, n int) (*network.Network, []types.Rewards) {
	...
	for i := 0; i < n; i++ {
		state.RewardsList = append(state.RewardsList, types.Rewards{
			Id: uint64(i),
		})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Rewards)
			}
		})
	}
}
...
func TestListRewards(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.Subset(t, objs, resp.Rewards)
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Rewards)
	})
}
